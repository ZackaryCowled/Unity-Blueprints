using System.Collections;
using System.Collections.Generic;
using UnityEngine;

[System.Serializable]
public class BlueprintNodeGetGetTransform : BlueprintNodeGet
{
	//Return the blueprnit node get get transform output attribute
	public override object GetAttribute()
	{
		//Return output attribute
		return BlueprintInstanceManager.GetBlueprintParentAt(blueprintID).transform;
	}

//If running in the Unity Editor
#if UNITY_EDITOR
	//Initializes the blueprint node get get transform instance
	public override void Initialize()
	{
		//Perform base initialization
		base.Initialize();

		//Initialize the blueprint node get get transform
		connections.Add(new BlueprintConnectionAttributeOutputTransform());
		connections[connections.Count - 1].Initialize(blueprintID, nodeID, 1, false);
	}

	//Returns the blueprint node get get transform title
	protected override string GetTitle()
	{
		//Return the blueprint node get get transform title
		return "GetTransform";
	}

	//Returns the blueprint node get get transform body components
	protected override void RenderBodyComponents()
	{
		//Render the Transform transform label
		BeginSection(1);
			GUILayout.Label("Transform : transform", BlueprintStyleHelper.GetNodeAttributeTextStyle());
		EndSection();
	}
#endif
}