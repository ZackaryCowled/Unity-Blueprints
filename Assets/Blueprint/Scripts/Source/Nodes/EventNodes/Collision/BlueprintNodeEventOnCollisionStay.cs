...
[System.Serializable]
public class BlueprintNodeEventOnCollisionStay : BlueprintNodeEvent
{
	//Start initializes the blueprint node event on collision stay instance
	public override void StartInitialize()
	{
		...
		if (connections[0].connectionNodeID > -1)
		{
			...
			if (gameObjectInputAttribute != null)
			{
				...
				//Setup the collision event handler to call the execute function on collision stay
				collisionEventHandler.onCollisionStayEvent.AddListener(Execute);
			}
		}
	}
...
//If running in the Unity Editor
#if UNITY_EDITOR
	//Initializes the blueprint node event on collision stay instance
	public override void Initialize()
	{
		//Perform base initiailzation
		base.Initialize();

		//Initialize the blueprint node event on collision stay
	...
	//Returns the blueprint node event on collision stay title
	protected override string GetTitle()
	{
		//Return the blueprint node event on collision stay title
		return "OnCollisionStay";
	}

	//Renders the blueprint node event on collision stay body components
