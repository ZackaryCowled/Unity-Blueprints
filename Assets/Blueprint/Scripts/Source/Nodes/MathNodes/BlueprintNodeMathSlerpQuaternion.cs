...
[System.Serializable]
public class BlueprintNodeMathSlerpQuaternion : BlueprintNodeMath
{
	//Returns the blueprint node math slerp Quaternion output attribute
	public override object GetAttribute()
	{
		...
		if (connections[0].connectionNodeID > -1 && connections[1].connectionNodeID > -1 && connections[2].connectionNodeID > -1)
		{
			...
			return Quaternion.Slerp((Quaternion)BlueprintInstanceManager.GetBlueprintAt(blueprintID).GetBlueprintNodeAt(connections[0].connectionNodeID).GetAttribute(),
	...
	//Returns the blueprint node math slerp Quaternion default output attribute
	public override object GetDefaultOutputAttribute()
	{
		//Return the blueprint node math slerp Quaternion default output attribute
		return Quaternion.identity;
	}

//If running in the Unity Editor
#if UNITY_EDITOR
	//Initializes the blueprint node math slerp Quaternion instance
	public override void Initialize()
	{
		//Perform base initialization
		base.Initialize();

		//Initialize the blueprint node math slerp Quaternion
	...
	//Returns the blueprint node math slerp Quaternion title
	protected override string GetTitle()
	{
		//Return the blueprint node math slerp Quaternion title
		return "SlerpQuaternion";
	}

	//Renders the blueprint node math slerp Quaternion body components
