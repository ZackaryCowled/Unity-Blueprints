...
[System.Serializable]
public class BlueprintNodeSetSetLocalPosition_TransformVector3 : BlueprintNodeSet
{
	...
	public override void Execute()
	{
		...
		if (connections[0].connectionNodeID > -1 && connections[1].connectionNodeID > -1)
		{
			...
			Transform transformInputAttribute = (Transform)BlueprintInstanceManager.GetBlueprintAt(blueprintID).GetBlueprintNodeAt(connections[0].connectionNodeID).GetAttribute();

			//If the transform input attribute is valid
			if (transformInputAttribute != null)
			{
				//Set the local position of the transform input attribute to the vector3 position input attribute
				transformInputAttribute.localPosition = (Vector3)BlueprintInstanceManager.GetBlueprintAt(blueprintID).GetBlueprintNodeAt(connections[1].connectionNodeID).GetAttribute();
			}
		}
