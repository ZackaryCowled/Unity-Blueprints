...
[System.Serializable]
public class BlueprintNodeSetSetLocalRotation_TransformQuaternion : BlueprintNodeSet
{
	...
	public override void Execute()
	{
		...
		if (connections[0].connectionNodeID > -1 && connections[1].connectionNodeID > -1)
		{
			...
			{
				//Set the local rotation of the transform input attribute to the Quaternion rotation input attribute
				transformInputAttribute.localRotation = (Quaternion)BlueprintInstanceManager.GetBlueprintAt(blueprintID).GetBlueprintNodeAt(connections[1].connectionNodeID).GetAttribute();
			}
		}
