...
[System.Serializable]
public class BlueprintNodeSetSetLocalRotation_TransformVector3 : BlueprintNodeSet
{
	...
	public override void Execute()
	{
		...
		if (connections[0].connectionNodeID > -1 && connections[1].connectionNodeID > -1)
		{
			...
			{
				//Set the local rotation of the transform input attribute to the vector3 rotation input attribute
				transformInputAttribute.localRotation = Quaternion.Euler((Vector3)BlueprintInstanceManager.GetBlueprintAt(blueprintID).GetBlueprintNodeAt(connections[1].connectionNodeID).GetAttribute());
			}
		}
