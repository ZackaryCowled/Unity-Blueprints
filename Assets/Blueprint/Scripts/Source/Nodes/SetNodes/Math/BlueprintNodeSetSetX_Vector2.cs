...
[System.Serializable]
public class BlueprintNodeSetSetX_Vector2 : BlueprintNodeSet
{
	//Executes the blueprint node set set x instance
	public override void Execute()
	{
		...
		if (connections[0].connectionNodeID > -1 && connections[1].connectionNodeID > -1)
		{
			...
			Vector2 inputAttribute = (Vector2)BlueprintInstanceManager.GetBlueprintAt(blueprintID).GetBlueprintNodeAt(connections[0].connectionNodeID).GetAttribute();

			//Set the x coordinate of the input attribute to the float input attribute
...
//If running in the Unity Editor
#if UNITY_EDITOR
	//Initializes the blueprint node set set x instance
	public override void Initialize()
	{
		//Perform base initialization
		base.Initialize();

		//Initialize the blueprint node set set x
	...
	//Returns the blueprint node set set x title
	protected override string GetTitle()
	{
		//Return the blueprint node set set x title
		return "SetX";
	}
	...
	protected override void RenderBodyComponents()
	{
		...
		BeginSection(1);
			GUILayout.Label("Vector2 : vector");
		EndSection();
