...
[System.Serializable]
public class BlueprintNodeSetSetInt_Animator : BlueprintNodeSet
{
	//Executes the blueprint node set set int instance
	public override void Execute()
	{
		...
		if (connections[0].connectionNodeID > -1 && connections[1].connectionNodeID > -1)
		{
			...
			{
				//Set the animator input attributes
				animatorInputAttribute.SetFloat((string)BlueprintInstanceManager.GetBlueprintAt(blueprintID).GetBlueprintNodeAt(connections[1].connectionNodeID).GetAttribute(),
												(int)BlueprintInstanceManager.GetBlueprintAt(blueprintID).GetBlueprintNodeAt(connections[2].connectionNodeID).GetAttribute());
			}
		}
...
//If running in the Unity Editor
	...
	//Renders the blueprint node set set int title
	protected override void RenderBodyComponents()
	{
		...
		BeginSection(3);
			GUILayout.Label("Int : value", BlueprintStyleHelper.GetNodeAttributeTextStyle());
		EndSection();
	}
#endif
}